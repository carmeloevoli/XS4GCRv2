...
double compute_source(std::shared_ptr<T> xs, double slope, double E_s) {
  ...
  auto q_s = GSL::gslQAGIntegration<double>(
      [&](double logy) {
        ...
        return std::pow(y, 1. - slope) * xs->get(H1, TARGET::H, y * E_0, E_s);
      },
      std::log(E_s / E_0), std::log(1e4 * E_s / E_0), 1000, 1e-3);
  return E_0 * q_s;
}

double compute_base_source(double slope, double E_s) {
  auto sigmapp = 30. * cgs::mbarn;
  auto xi = 0.05;
  auto E_0 = cgs::GeV;
  auto E_p = E_s / xi;
  return sigmapp / xi * std::pow(E_p / E_0, -slope);
}
...
void print_secondary_source(std::shared_ptr<T> xs, std::string filename) {
  ...
  auto E_secondary = UTILS::LogAxis(10. * cgs::GeV, 10. * cgs::TeV, 3 * 32);
  for (auto& E_s : E_secondary) {
    outfile << E_s / cgs::GeV << "\t";
    outfile << compute_source(xs, 2.0, E_s) / cgs::mbarn << "\t";
    outfile << compute_source(xs, 2.4, E_s) / cgs::mbarn << "\t";
    outfile << compute_source(xs, 2.8, E_s) / cgs::mbarn << "\t";
    outfile << compute_base_source(2.8, E_s) / cgs::mbarn << "\t";
    outfile << "\n";
  }
  outfile.close();
}
...
int main() {
  try {
    ...
    //(SecondaryLeptonModels::ORUSA2022, "Orusa2022_pos");
    // }
