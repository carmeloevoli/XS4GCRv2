...
Evoli2019::Evoli2019() { init(); }

void Evoli2019::print() const {
  LOGI << "using Evoli2019 fragmentation model: Evoli et al. 2019 fit with ghost contributions";
}

void Evoli2019::init() {
  if (!UTILS::fileExists(fitDataFilename)) throw std::runtime_error("missing Evoli2019 fit-data file");
  if (!UTILS::fileExists(fitParamsFilename)) throw std::runtime_error("missing Evoli2019 fit-parameter file");
  if (!UTILS::fileExists(ghostListFilename)) throw std::runtime_error("missing Evoli2019 ghost-list file");
...
std::shared_ptr<Fragmentation> Evoli2019::clone() { return std::make_shared<Evoli2019>(*this); }

bool Evoli2019::hasChannel(const FragmentationChannel& ch) const {
  std::set<PID> activeGhosts;
  return hasChannel(ch, activeGhosts);
}

bool Evoli2019::hasChannel(const FragmentationChannel& ch, std::set<PID> activeGhosts) const {
  ...
  if (!activeGhosts.insert(child).second) {
    throw std::runtime_error("cycle detected in Evoli2019 ghost tree at isotope " + child.str());
  }
...
double Evoli2019::getTotal(const FragmentationChannel& ch, const TARGET& target, const double& T_n, bool do_ghosts) {
  if (T_n <= 0.) return 0.;
  if (target == TARGET::He) return 0.;
  if (target != TARGET::H) throw std::runtime_error("target not implemented in Evoli2019");

  double value = do_ghosts ? withGhosts(ch, T_n) : direct(ch, T_n);
  return std::max(value, 0.);
}

double Evoli2019::bestfitNormalization(const FragmentationChannel& ch) const {
...
double Evoli2019::direct(const FragmentationChannel& ch, double T_n) const {
...
double Evoli2019::withGhosts(const FragmentationChannel& ch, double T_n) const {
  std::set<PID> activeGhosts;
  return withGhosts(ch, T_n, activeGhosts);
}

double Evoli2019::withGhosts(const FragmentationChannel& ch, double T_n, std::set<PID> activeGhosts) const {
  const PID& projectile = ch.first;
  const PID& child = ch.second;
