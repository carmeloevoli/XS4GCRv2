...
// Kelner et al. (2006) gamma/neutrino production parametrization (formerly models/gammaKelner06).
namespace {
...
void Kelner2006Neutrals::print() const {
  LOGI << "using KELNER2006 neutral production model: ";
  LOGI << "Kelner et al., 2006, Physical Review D, vol.74, Issue 3, id.034018 ";
}

std::shared_ptr<SecondaryNeutrals> Kelner2006Neutrals::clone() { return std::make_shared<Kelner2006Neutrals>(*this); }

double Kelner2006Neutrals::getDifferential(const PID& projectile, const TARGET& target, const double& T_proj,
                                           const double& x) const {
  if (x > 1.) return 0.;
  double value = 0.;
  if (projectile == H1 && target == TARGET::H) {
    if (m_type == NeutralParticleType::GAMMA) {
      value = sigma_gamma(T_proj, x);
    } else if (m_type == NeutralParticleType::ALLNUS) {
      value = sigma_neutrinos(T_proj, x);
    }
  }
  return value;
}
