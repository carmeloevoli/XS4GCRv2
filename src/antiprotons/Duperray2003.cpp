...
// Duperray et al. (2003) antiproton production parametrization (formerly models/apDuperray03).
namespace {

enum Reaction { PP, PHE };

// Table II. Page 3
double C[11] = {0, 0.16990, 10.28, 2.269, 3.707, 0.009205, 0.4812, 3.3600, 0.063940, -0.1824, 2.4850};

// Table V. Page 8
double D[8] = {0., 3.4610, 4.340, 0.007855, 0.5121, 3.6620, 0.023070, 3.2540};
...
double dsigma_dT(double T_n, double T_ap, Reaction channel) {
  using std::sqrt;
...
double Duperray2003SecAp::getDifferential(const PID& particle, const TARGET& target, const double& T_proj,
                                          const double& x) const {
  const double T_ap = x * T_proj;
  double value = 0;
  if (particle == H1 && target == TARGET::H)
    value = dsigma_dT(T_proj, T_ap, PP);
  else if (particle == He4 || target == TARGET::He)
    value = dsigma_dT(T_proj, T_ap, PHE);
  return T_proj * value;
}
