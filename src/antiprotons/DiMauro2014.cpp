...
// Di Mauro et al. (2014) antiproton production parametrization (formerly models/apDiMauro14).
namespace {
...
double DiMauro2014SecAp::getDifferential(const PID& particle, const TARGET& target, const double& T_proj,
                                         const double& x) const {
  const double T_ap = x * T_proj;
  if (particle == H1 && target == TARGET::H)
    return T_proj * dsigma_dT(T_proj, T_ap);
  else
    return 0;
}
