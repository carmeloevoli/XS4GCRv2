...
void get_secondary_production(SecondaryLeptonModels model, double T_proj, std::string filename) {
  ...
  auto T_pos = UTILS::LogAxis(0.1 * cgs::GeV, T_proj, 100);
  for (auto T : T_pos) {
...
int main() {
  try {
    XS4GCR::LOG::startup_information();
    {
      ...
      get_secondary_production(XS4GCR::KAMAE2006, T_proj, "output/Kamae2006_1TeV_ap.txt");
      get_secondary_production(XS4GCR::HUANGPOHL2007, T_proj, "output/HuangPohl2007_1TeV_ap.txt");
    }
