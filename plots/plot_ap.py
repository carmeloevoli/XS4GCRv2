...
def set_axes(fig, title):
    ...
    ax.set_xlim([1e-3, 1])
    ax.set_xscale('log')
    ax.set_ylabel(r'$\sigma_{pp \rightarrow \overline{p}}$ [mbarn]')
    ax.set_yscale('log')
    ax.set_title(title)
    return ax
...
#    plot_model(ax, 'output/AAFRAG_100GeV_ap.txt', 'k', 'AAfrag', 1e3)
...
#    plot_model(ax, 'output/AAFRAG_1TeV_ap.txt', 'k', 'AAfrag', 1e3)

#    E, sigma = np.loadtxt('spec_aprot.txt', usecols=(0,1), unpack=True, skiprows=0)
#    ax.plot(E / max(E), sigma / E, color='k')
#    plot_model(ax, 'output/AAfrag_1TeV_ap.txt', 'k', 'AAfrag')
